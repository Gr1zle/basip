...
namespace Basip
{
    public class Worker : BackgroundService
    {
        ...
        private string GetApplicationVersion()
        {
            ...
            {
                logger.LogCritical($"Version detection: {ex.Message}");
            }
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            logger.LogCritical(@$"32 basip start: {timestart} deltasleep: {deltasleep}");
            logger.LogCritical(@$"33 Service basip write and delete card started");
            await Task.Delay(deltasleep);
            while (!stoppingToken.IsCancellationRequested)
            {
                logger.LogCritical($@"Старт итерации");
                try
                {
                    logger.LogCritical($"Версия приложения basip: {version}");
                ...
                logger.LogCritical($@"timeout basip: {timeout}");
        ...
        private void run()
        {
            ...
            logger.LogCritical("Ok connect database");
                            ...
                            if (options.clear_log)
                            {
                                ...
                                switch (statusLog.StatusCode)
                                {
                                    case HttpStatusCode.OK:
                                        logger.LogCritical($"Successful attempt to clear the event log IP: {dev.base_url}");
                                        break;
                                    default:
                                        logger.LogCritical($"Failed attempt to clear the event log IP: {dev.base_url}");
                                        break;
                                }
                            }
                            else
                            {
                                logger.LogCritical($"The event log has not been cleared IP: {dev.base_url}");
                            }
        ...
        private async Task ProcessCardWrite(DB db, Device dev, DataRow card, int deviceId)
        {
            ...
            logger.LogCritical($@"Command destination: writekey id_dev={deviceId} BASE_URL {dev.base_url} key=""{options.uidtransform(cardId)}"" AddCard ");
        ...
        private async Task ProcessCardWriteSuccess(DB db, Device dev, DataRow card, string cardId, int deviceId, RestResponse request)
        {
            try
            {
                ...
                logger.LogCritical($@"Answer destination: writekey id_dev={deviceId} BASE_URL {dev.base_url} Answer: OK key=""{options.uidtransform(cardId)}"" uid={uid}");
        ...
        private async Task ProcessLogEvent(DB db, Device dev, Basip.DeviceLibrary.LogItem logEvent, int deviceId)
        {
            ...
            try
            {
                ...
                string note = $"Device=\"{dev.ControllerName}\", Type={eventType} Readdate=#{DateTimeOffset.Now:yyyy-MM-dd HH:mm:ss}# DeviceDate =#{dateTime}# DeviceDate = {logEvent.timestamp}";

                if (cardNumber != null)
                {
                    logger.LogDebug($"+{note}, Card=\"{cardNumber}\"");
                }
                else {
                ...
                if (eventCode > 0)
                    ...
                    if (success) 
                    {
                        logger.LogDebug($"Event saved: Dev={deviceId}, Type={eventType}, Code={eventCode}");
                    }
                    else
                    {
