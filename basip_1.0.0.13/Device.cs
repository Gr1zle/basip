    ...
    public class Device
    {
        ...
        public async Task<RestResponse> GetEvents(long fromTimestamp = 0, int limit = 50)
        {
            ...
            if (fromTimestamp > 0)
            {
                ...
                long fromTimestampSeconds = fromTimestamp;

                request.AddQueryParameter("from", fromTimestampSeconds.ToString());
            }
        ...
        public async Task<RestResponse> ClearingLog()
        {
            ...
            request.AddStringBody(data.ToJsonString(), ContentType.Json);
            request.AddHeader("Accept", "application/json");
            request.AddHeader("Content-Type", "application/json");
            request.AddHeader("Authorization", "Bearer " + token);
