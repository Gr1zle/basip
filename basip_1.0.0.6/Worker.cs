    ...
    public class Worker : BackgroundService
    {
        ...
        private async Task TaskGet(DataRow row)// в row содержатс€ логин, пароль, id_dev, IP адрес
        {
           ...
           // dev.base_url = "http://192.168.8.102:8888";
            ...
            logger.LogTrace($"ѕанель ID: {row["id_dev"]}, IP: {dev.ip} - Card count: " + cardList.Count);
                            ...
                            case HttpStatusCode.BadRequest:
                                ...
                                try
                                {
                                    int rowsUpdated = db.FixCardIdxOK(
                                        card["id_card"].ToString(),
                                        row["id_dev"].ToString(),
                                        "0", // —пециальное значение дл€ уже существующих карт
                                        con
                                    );
