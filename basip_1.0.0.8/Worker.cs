...
namespace Basip
{
    public class Worker : BackgroundService
    {
        ...
        private async Task TaskGet(DataRow row)// в row содержатся логин, пароль, id_dev, IP адрес
        {
            ...
            logger.LogTrace($"Панель ID: {row["id_dev"]}, IP: {dev.ip} - Card count: " + cardList.Count);
                        ...
                        switch (request.StatusCode)
                        {
                            case HttpStatusCode.OK:
                                ...
                                try
                                {
                                    ...
                                    {
                                        logger.LogWarning($"No rows updated in CARDIDX for card {card["id_card"]}");
                                    }
                                }
                                catch (Exception ex)
                                {
                            ...
                            case HttpStatusCode.BadRequest:
                                logger.LogDebug($@"Answer destination: writekey id_dev={row["id_dev"]} BASE_URL {dev.base_url} Answer: BAD REQUEST key=""{options.uidtransform(card["id_card"].ToString())}"" card already exists");
                                ...
                                try
                                {
                                    ...
                                    {
                                        logger.LogWarning($"No rows updated in CARDIDX for existing card {card["id_card"]}");
                                    }
                                }
                                catch (Exception ex)
                                {
